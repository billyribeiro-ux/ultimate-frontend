...
**Why this matters at scale.** In a production application with 50+ components, every component that renders text or has spacing makes a choice: hard-coded values or fluid tokens. If even a few components use hard-coded `px` sizes, you end up with a patchwork where some elements breathe with the viewport and others snap. The visual inconsistency is subtle but cumulative — users perceive the app as "janky" without knowing why. Fluid tokens enforced at the design-system level mean that every component, written by any team member, automatically participates in the same smooth scaling behaviour. This is a governance problem as much as a technical one, and `clamp()` tokens solve it by making the right choice the default choice.

**The mental model.** Think of `clamp(min, preferred, max)` as a rubber band stretched between two pegs (the min and max). The preferred value is where your finger is pulling. When the viewport is narrow, the rubber band hits the left peg and stops shrinking. When the viewport is wide, it hits the right peg and stops growing. Between the pegs, the band stretches linearly. This mental image helps you reason about edge cases: if your preferred coefficient is too aggressive (say, `12vw`), the rubber band reaches the max peg very early and the value is clamped for most viewport widths — effectively wasting the fluid range.

**Edge cases.** A common silent failure: using `px` for the min and max instead of `rem`. The values look fine until a user increases their browser's base font size for accessibility. At that point `rem`-based values scale up but `px` values do not, so your clamp's floor and ceiling become mismatched with the rest of the type scale. Another edge case: `clamp()` with a negative preferred slope (where the value should *shrink* as the viewport grows). This is rare but legitimate for spacing in sidebars that should compress on wide screens to give content more room. TypeScript cannot catch these CSS-level mistakes, so linting tools like Stylelint with a custom rule for fluid tokens are valuable at scale.

**Performance implications.** `clamp()` is evaluated by the browser's layout engine on every reflow that changes the viewport size. In practice this is negligible — CSS math functions are hardware-optimised and the browser is already recalculating layout during a resize. There is no JavaScript involved, no event listener, no requestAnimationFrame loop. The fluid effect is entirely CSS-native. This matters for bundle size: a fluid token system adds zero JavaScript bytes. Compare this to JavaScript-driven responsive sizing (window resize listeners, state updates, re-renders) which add both code weight and CPU work. A pure-CSS fluid system is strictly superior on every performance axis.
