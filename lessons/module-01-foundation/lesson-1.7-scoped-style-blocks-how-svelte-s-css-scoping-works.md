...
**Why this matters at scale.** In a 50-component application, global CSS becomes a minefield. Every developer adding a class name must mentally check whether that name already exists in any other file. With scoped styles, that mental overhead drops to zero — you name classes based on what they mean *in this component*, not based on what might collide elsewhere. Teams that adopt scoped styles report fewer CSS bugs, faster onboarding (new developers do not need to memorise naming conventions), and dramatically smaller diffs during refactors because renaming a class only touches one file.

**The mental model.** Think of each component's `<style>` block as a locked room. The CSS rules inside can see and affect only the furniture (elements) in that room. They cannot reach through walls into neighbouring rooms. If you need to affect something in another room, you either hand it a key (a CSS custom property passed as a prop) or you use the `:global()` intercom, which broadcasts through all walls. The room metaphor also explains why child components are not affected: they are their own rooms with their own locks.

**Edge cases.** The most surprising edge case is dynamic elements: content rendered via `{@html rawString}` does not get the scoping hash because the compiler cannot see those elements at compile time. Styles you write will not apply to `{@html}` content unless you use `:global()`. Another subtle case: elements conditionally rendered via `{#if}` or `{#each}` *do* get the hash, because the compiler can statically analyse the template. A third edge case involves CSS combinators: `.parent > .child` becomes `.parent.svelte-hash > .child.svelte-hash`, which means both parent *and* child must be in this component's markup. If `.child` is actually rendered by a child component, the selector silently fails to match.

**Performance implications.** Scoped styles add roughly 8-12 bytes per element (the hash class) and a similar amount per selector in the CSS output. For a 50-component app this might total 2-4KB of extra CSS — trivial. The runtime cost is exactly zero: no JavaScript runs to apply or manage scoping. The browser's native selector matching is extremely fast, and the compound selectors (`.card.svelte-hash`) are no slower than simple class selectors in practice. Compare this to CSS-in-JS solutions that ship 10-30KB of runtime JavaScript and inject styles dynamically, causing layout thrashing on first paint. Svelte's approach is measurably faster on every Core Web Vital.
