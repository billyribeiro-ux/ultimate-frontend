...
**Why this matters at scale.** In a production codebase with 50+ components, the component boundary is your primary unit of reasoning. When a bug report says "the metric card shows stale data," you know exactly which file to open. When a design change says "cards should have rounded corners," you change one file and every card in the app updates. Without components, a 50-page application is 50 long files where knowledge is scattered and duplicated. With components, it is a library of 30-80 focused files that compose into pages. The productivity difference is not 2x — it is 10x, because the cost of change scales linearly with components but quadratically with copy-paste.

**The mental model.** Think of a component as a stamp. The stamp has a fixed shape (the markup), a fixed colour scheme (the scoped styles), and a few blanks you can fill in (the props, which you will learn in Lesson 3.2). Every time you press the stamp onto a page, you get the same shape but with different text in the blanks. If the stamp design needs to change, you modify the stamp itself — not the hundreds of impressions it has already made. This metaphor holds even at scale: a design system is a collection of stamps, and a product team's job is to compose stamps onto pages without modifying them.

**Edge cases.** A common mistake is making components too small. A `<Text>` component that wraps a single `<p>` tag adds indirection without value — the native element already does the job. The threshold question is: "does this component isolate a *decision* or just rename an element?" If it isolates a decision (styling, behaviour, accessibility attributes), it is worth the file. If it is just a rename, delete it. Another edge case: circular imports. If component A renders component B and component B renders component A, the bundler will either error or produce undefined behaviour. Component trees must be acyclic, just like the reactive dependency graph.

**Performance implications.** Each Svelte component instance carries a small overhead: roughly 200-500 bytes of compiled code for the component's lifecycle management. For typical applications (50-200 component instances on a page), this is unmeasurable. For extreme cases (rendering 5,000 component instances in a list), the overhead adds up and you should consider inlining the markup or using a single component with an `{#each}` block. The compiled output of a Svelte component is a lean JavaScript class — no virtual DOM, no reconciler, no fiber tree. The per-component cost is among the lowest of any framework.
