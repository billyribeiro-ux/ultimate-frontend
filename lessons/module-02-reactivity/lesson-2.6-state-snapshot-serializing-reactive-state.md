...
**Why this matters at scale.** In production apps, state constantly crosses boundaries: it gets sent to analytics services, persisted to localStorage, transmitted via `postMessage` to web workers, serialized into URL query parameters, or passed to third-party charting libraries. Every one of these boundaries expects plain JavaScript objects. A 50-component app might have 10-15 serialization points. Without a consistent "extract plain copy" tool, each boundary develops its own ad-hoc solution — some use spread, some use `JSON.parse(JSON.stringify(...))`, some just pass the proxy and hope. `$state.snapshot` standardizes this, making every serialization point safe and predictable.

**The mental model.** Think of `$state.snapshot` as a photocopy machine. You put the original (live, reactive, possibly being edited by someone else right now) onto the glass, press the button, and get a flat paper copy. The copy captures the state at that exact moment. Future edits to the original do not appear on the copy. Edits to the copy do not affect the original. The copy is suitable for filing, mailing, or handing to someone who does not need real-time updates. This is exactly what serialization needs: a frozen-in-time, standalone representation.

**Edge cases.** `$state.snapshot` does not preserve prototype chains or class instances — it produces plain objects and arrays. If your state contains a `Date` instance, the snapshot will have a plain object (or string, depending on the serialization path), not a `Date`. Design your state interfaces to use serializable primitives (ISO strings for dates, numbers for timestamps) rather than class instances when you know you will snapshot. Another edge case: circular references. If your reactive state has circular object references, `$state.snapshot` will throw — just like `JSON.stringify` would. Ensure your state trees are acyclic. Finally, snapshotting a `$state.raw` value returns the value as-is (it is already plain), so the operation is essentially free for raw state.

**Performance implications.** `$state.snapshot` performs a deep clone — every nested object and array is copied. For small state (under 100 fields total), this is sub-millisecond. For large state (10,000+ nodes), it can take 1-5ms. Profile before snapshotting large trees in hot paths like scroll handlers or animation frames. In most applications, snapshots happen on user actions (save button, export, navigation) where a few milliseconds are imperceptible. Never snapshot inside a `$derived` expression — that creates a new deep copy on every reactive update, which defeats the caching purpose of `$derived`.
