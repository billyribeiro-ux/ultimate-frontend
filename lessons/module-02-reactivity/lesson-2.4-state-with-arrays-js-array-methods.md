...
**Why this matters at scale.** Lists dominate production UIs. A typical SaaS dashboard has 5-15 list-rendering components: tables, notification feeds, sidebar menus, dropdown options, autocomplete suggestions. In a 50-component app, if even one developer misunderstands how array mutations trigger reactivity, you get phantom bugs: items that do not appear until a page refresh, delete buttons that seem to work but leave ghost DOM nodes, sort operations that desynchronize the visual order from the data order. Mastering array state means mastering the most common state shape in real applications.

**The mental model.** Think of a reactive array as a ledger with line numbers. When the UI reads line 3, it subscribes to line 3. When you `splice(3, 1)`, Svelte knows that line 3 was removed and lines 4+ shifted down. It updates only the DOM nodes affected by the shift. When you `push(item)`, a new line is added at the end and only a new DOM node is created — existing nodes are untouched. This ledger model explains why keyed `{#each}` blocks (Module 4) matter: keys tell Svelte which ledger line a DOM node corresponds to, so that reordering the ledger correctly reorders the DOM instead of recreating it.

**Edge cases.** The `sort()` and `reverse()` methods mutate in place and trigger reactivity, but because they potentially move every element, Svelte has to reconcile the entire list. Without keys, this means every DOM node may be reused for a different item. With keys, Svelte can efficiently reorder the existing DOM nodes. Another edge case: `todos.length = 0` is a valid way to clear an array and does trigger reactivity, but it can confuse developers who expect only method calls to be intercepted. Direct index assignment (`todos[5] = newItem`) also works — the proxy intercepts numeric property sets — but TypeScript may warn if the index could be out of bounds. Finally, nested array mutations (`todos[0].subtasks.push(...)`) work because the proxy is deep, but the update notification is scoped to the subtask array, not the parent todo array.

**Performance implications.** For arrays under 1,000 items, Svelte's proxy-based reactivity is effectively zero-cost. The proxy intercepts `push`/`splice`/etc. and emits a targeted update in O(1) for additions and O(n) for sort/reverse (where n is the number of affected elements). For arrays above 5,000 items, consider `$state.raw` (Lesson 2.5) combined with `$derived` for filtered views, or use virtual scrolling (Module 12) to limit the number of rendered DOM nodes. The key insight: reactivity cost scales with the number of *reactive reads* (subscriptions), not the number of items in the array. If you render 10,000 items but only 50 are visible in a virtual scroller, you have ~50 subscriptions, not 10,000.
